...
static void ondemandx_early_suspend(struct power_suspend *handler) {
       ondemandx_suspend(1);
}

static void ondemandx_late_resume(struct power_suspend *handler) {
       ondemandx_suspend(0);
}

static struct power_suspend ondemandx_power_suspend = {
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		dbs_timer_init(this_dbs_info);
                register_power_suspend(&ondemandx_power_suspend);
	...
	case CPUFREQ_GOV_STOP:
		...
		if (!dbs_enable)
			sysfs_remove_group(cpufreq_global_kobject,
					   &dbs_attr_group);
                unregister_power_suspend(&ondemandx_power_suspend);
